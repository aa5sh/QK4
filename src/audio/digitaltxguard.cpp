...
QString calibrationKey(int mode) {
    return mode == int(DigitalTxGuard::Mode::Sstv)
               ? QStringLiteral("digitalTx/calibration/sstv")
...
bool DigitalTxCalibration::save(QSettings &settings, int mode, float gain) {
    ...
    settings.setValue(calibrationKey(mode),
                      QVariantMap{{QStringLiteral("version"), 1}, {QStringLiteral("gain"), gain},
...
bool DigitalTxGuard::begin(Mode mode, quint64 generation, qint64 now, bool calibration) {
    if (m_active || m_latched || generation == 0) return false;
    m_mode = mode;
    ...
    if (calibration) m_control->gain.store(CalibrationStartGain, std::memory_order_release);
...
void DigitalTxGuard::acknowledge() {
    if (!m_active) { m_latched = false; m_reason.clear(); }
}

void DigitalTxGuard::audioAccepted(qint64 now) {
    if (!m_active) return;
    if (m_firstAudio < 0) m_firstAudio = now;
    m_lastAudio = now;
}

DigitalTxGuard::Action DigitalTxGuard::trip(const QString &reason) {
    if (!m_active) return Action::None;
    stop();
    m_latched = true;
...
DigitalTxGuard::Action DigitalTxGuard::meter(const QString &command, qint64 now) {
    if (!m_active || !command.startsWith(QStringLiteral("TM")) || command == QStringLiteral("TM1")
        || command == QStringLiteral("TM0")) return Action::None;
    if (command.size() != 14)
        return trip(QStringLiteral("TX stopped: invalid K4 meter response."));
    for (int i = 2; i < command.size(); ++i)
        if (!command[i].isDigit()) return trip(QStringLiteral("TX stopped: invalid K4 meter response."));
    m_lastMeter = now;
    ...
    m_meterDiagnostic = QStringLiteral("raw ALC %1 · drive %2 dB")
                            .arg(alc).arg(20.0 * std::log10(qMax(gain(), MinimumGain)), 0, 'f', 1);
    ...
    if (alc < ReduceAlc) {
        m_highSince = -1;
        m_highSamples = 0;
        if (!m_calibrating) return Action::None;
        if (alc >= 3) {
            if (m_stableSince < 0) m_stableSince = now;
            if (now - m_stableSince >= 1200) return Action::Calibrated;
    ...
    m_stableSince = -1;
    if (m_highSince < 0) m_highSince = now;
    ++m_highSamples;
    if (m_lastReduction >= 0 && now - m_lastReduction < 400) return Action::None;
    m_control->gain.store(qMax(MinimumGain, gain() * .70710678f), std::memory_order_release);
    m_adjusted = m_lastReduction = now;
    m_reduced = true;
    if (m_calibrating) { m_highSince = -1; m_highSamples = 0; }
