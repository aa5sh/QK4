...
HardwareController::HardwareController(RadioState *radioState, ConnectionController *connController, QObject *parent)
    ...
    const auto document = QJsonDocument::fromJson(
        ctr2Settings.value(QStringLiteral("ctr2/mapping")).toByteArray());
    ...
    connect(m_midiInputRouter, &MidiInputRouter::knobActionRequested,
            this, &HardwareController::ctr2KnobActionRequested);
    connect(m_midiInputRouter, &MidiInputRouter::buttonActionRequested,
            this, &HardwareController::ctr2ButtonActionRequested);
    connect(m_midiInputRouter, &MidiInputRouter::macroRequested, this,
            [this](const QString &, const QString &command) {
        if (m_connectionController->isConnected()) m_connectionController->sendCAT(command);
    });
    const QString savedCtr2 = ctr2Settings.value(QStringLiteral("ctr2/port")).toString();
    if (!savedCtr2.isEmpty()) QTimer::singleShot(0, this, [this, savedCtr2] { connectCtr2(savedCtr2); });
...
HardwareController::~HardwareController() {
    ...
    if (m_ctr2MidiDevice) m_ctr2MidiDevice->closePort();
...
MidiMapping::DeviceMapping HardwareController::ctr2Mapping() const {
    return m_midiInputRouter ? m_midiInputRouter->mapping(QStringLiteral("ctr2-midi"))
                             : MidiMapping::ctr2Default();
}

void HardwareController::setCtr2Mapping(const MidiMapping::DeviceMapping &mapping) {
    if (!m_midiInputRouter) return;
...
bool HardwareController::connectCtr2(const QString &portName) {
    if (!m_ctr2MidiDevice) return false;
    if (portName.isEmpty()) {
        m_ctr2MidiDevice->closePort();
        QSettings().remove(QStringLiteral("ctr2/port"));
        return true;
    }
    if (!m_ctr2MidiDevice->openPort(portName)) return false;
