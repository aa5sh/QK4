...
QDateTime adifDateTime(const AdifRecord &record, const char *dateKey, const char *timeKey) {
    ...
    while (time.size() < 6) time.append(QLatin1Char('0'));
...
bool WsjtBroadcaster::send(const QByteArray &packet) {
    QSettings settings;
    if (!settings.value(QStringLiteral("wsjtUdp/enabled"), false).toBool()) return false;
    const QHostAddress address(settings.value(QStringLiteral("wsjtUdp/address"),
...
void WsjtBroadcaster::sendStatus(qint64 frequency, const QString &mode, bool transmitting,
                                 int rxHz, int txHz, const QString &myCall, const QString &myGrid) {
    QByteArray packet = header(1);
    QDataStream out(&packet, QIODevice::Append);
    out.setByteOrder(QDataStream::BigEndian);
    out.setVersion(QDataStream::Qt_5_4);
    out << quint64(qMax<qint64>(0, frequency)) << mode.toUtf8() << QByteArray() << QByteArray()
        << mode.toUtf8() << true << transmitting << false << quint32(rxHz) << quint32(txHz)
        << myCall.toUtf8() << myGrid.toUtf8() << QByteArray() << false << QByteArray()
        << false << quint8(0) << quint32(0) << quint32(mode == QStringLiteral("FT4") ? 8 : 15)
...
void WsjtBroadcaster::sendDecode(const Ft8::Decode &decode) {
    ...
    out.setByteOrder(QDataStream::BigEndian);
    out.setVersion(QDataStream::Qt_5_4);
    out << true << decode.utc.toUTC().time() << qint32(decode.snr.value_or(0)) << decode.dt
        << quint32(decode.audioHz) << Ft8::modeName(decode.mode).toUtf8() << decode.message.toUtf8()
        << false << false;
    send(packet);
}
...
void WsjtBroadcaster::sendQsoLogged(const AdifRecord &record) {
    ...
    out << off << record.value(QStringLiteral("CALL")).toUtf8()
        << record.value(QStringLiteral("GRIDSQUARE")).toUtf8() << dialHz(record)
        << record.value(QStringLiteral("MODE")).toUtf8()
        << record.value(QStringLiteral("RST_SENT")).toUtf8()
        << record.value(QStringLiteral("RST_RCVD")).toUtf8()
        << record.value(QStringLiteral("TX_PWR")).toUtf8()
        << record.value(QStringLiteral("COMMENT")).toUtf8()
        << record.value(QStringLiteral("NAME")).toUtf8() << on
        << record.value(QStringLiteral("OPERATOR")).toUtf8()
        << record.value(QStringLiteral("STATION_CALLSIGN")).toUtf8()
        << record.value(QStringLiteral("MY_GRIDSQUARE")).toUtf8()
        << record.value(QStringLiteral("STX_STRING")).toUtf8()
        << record.value(QStringLiteral("SRX_STRING")).toUtf8()
        << record.value(QStringLiteral("PROP_MODE")).toUtf8();
    send(packet);
}
