...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), m_radioState(new RadioState(this)) {
    ...
    connect(m_hardwareController, &HardwareController::ctr2KnobActionRequested,
            this, &MainWindow::handleCtr2Knob);
    connect(m_hardwareController, &HardwareController::ctr2ButtonActionRequested,
            this, &MainWindow::handleCtr2Button);
...
void MainWindow::handleCtr2Knob(const QString &action, int value, bool absolute) {
    if (m_digitalModesController->handleCtr2Knob(action, value, absolute)
        || action == QStringLiteral("disabled") || (!absolute && value == 0)) return;
    const auto scaled = [absolute, value](int low, int high, int current, int step) {
        return absolute ? low + qRound((high - low) * value / 127.0)
                        : qBound(low, current + value * step, high);
    };
    ...
    if (!m_connectionController->isConnected()) return;
    ...
    if (action == QStringLiteral("active_vfo_frequency") || action == QStringLiteral("other_vfo_frequency")) {
        bool tuneB = targetB;
        if (action == QStringLiteral("other_vfo_frequency")) tuneB = !tuneB;
        const quint64 current = tuneB ? m_radioState->vfoB() : m_radioState->vfoA();
        const int step = RadioUtils::tuningStepToHz(tuneB ? m_radioState->tuningStepB()
                                                         : m_radioState->tuningStep());
        const qint64 next = qint64(current) + qint64(value) * step;
        if (next > 0) {
            const QString command = QStringLiteral("%1%2;").arg(tuneB ? QStringLiteral("FB")
                                                                       : QStringLiteral("FA"))
        ...
        const double next = absolute ? .1 + 109.9 * value / 127.0
                                     : qBound(.1, current + value * (current <= 10 ? .1 : 1.0), 110.0);
        ...
        m_connectionController->sendCAT(QStringLiteral("BW%1%2;").arg(targetB ? QStringLiteral("$") : QString())
        ...
        for (int i = 0; i < qMin(qAbs(value), 64); ++i) m_connectionController->sendCAT(command);
    }
}

void MainWindow::handleCtr2Button(const QString &action) {
    if (m_digitalModesController->handleCtr2Button(action) || !m_connectionController->isConnected()) return;
    const bool b = m_radioState->bSetEnabled();
    QString command;
    if (action == QStringLiteral("mode_next")) command = b ? QStringLiteral("MD$+;") : QStringLiteral("MD+;");
    else if (action == QStringLiteral("mode_previous")) command = b ? QStringLiteral("MD$-;") : QStringLiteral("MD-;");
    else if (action == QStringLiteral("band_up")) command = b ? QStringLiteral("BN$+;") : QStringLiteral("BN+;");
    else if (action == QStringLiteral("band_down")) command = b ? QStringLiteral("BN$-;") : QStringLiteral("BN-;");
    else if (action == QStringLiteral("nr_toggle")) command = QStringLiteral("SW62;");
    else if (action == QStringLiteral("rit_toggle")) command = b ? QStringLiteral("RT$/;") : QStringLiteral("RT/;");
    else if (action == QStringLiteral("split_toggle")) command = QStringLiteral("FT/;");
    else if (action == QStringLiteral("tune_step")) command = b ? QStringLiteral("VT$/;") : QStringLiteral("VT/;");
    ...
    if (!command.isEmpty()) m_connectionController->sendCAT(command);
}
